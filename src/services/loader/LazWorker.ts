...
async function initializeFile(fileBuffer: ArrayBuffer, batchSizeParam: number = 500) {
  try {
    // Read header
  ...
  try {
    ...
    for (let i = 0; i < batchPointCount; i++) {
      try {
        ...
        const scaledX = x * (currentHeader.ScaleFactorX || 1) + (currentHeader.OffsetX || 0);
        const scaledY = y * (currentHeader.ScaleFactorY || 1) + (currentHeader.OffsetY || 0);
        const scaledZ = z * (currentHeader.ScaleFactorZ || 1) + (currentHeader.OffsetZ || 0);
