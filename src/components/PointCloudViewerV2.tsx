...
interface PointCloudViewerV2Props {
  className?: string;
}

export const PointCloudViewerV2: React.FC<PointCloudViewerV2Props> = ({ className }) => {
  ...
  const handlePointCloudLoaded = (data: any) => {
    setIsLoading(false);
    setError(null);
  ...
  const handlePointCloudLoading = (data: any) => {
    setIsLoading(true);
    setError(null);
  };
  ...
  const updatePointCloudList = () => {
    if (serviceManagerRef.current) {
      setPointCloudIds(serviceManagerRef.current.getPointCloudIds());
    }
  };
  ...
  const handleRenderOptionChange = (option: keyof RenderOptions, value: any) => {
    ...
    serviceManagerRef.current.updateRenderOptions(newOptions);
  };

  const handlePointCloudSelect = (id: string) => {
    if (serviceManagerRef.current) {
      serviceManagerRef.current.setActivePointCloud(id);
    }
  };
