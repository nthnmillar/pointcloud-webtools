...
export class VoxelDownsampleDebugBERust extends BaseService {
  ...
  private pendingRequests = new Map<
    string,
    {
      resolve: (value: VoxelDownsampleDebugBERustResult) => void;
      ...
      this.ws.onmessage = async event => {
        try {
          ...
          if (event.data instanceof ArrayBuffer) {
            ...
            if (
              this.pendingHeader &&
              this.pendingHeader.type === 'voxel_debug_rust_result' &&
            ...
            if (message.type === 'voxel_debug_rust_result') {
              if (message.success && message.dataLength) {
                ...
                if (pending) {
                  ...
                  pending.reject(
                    new Error(
                      error || 'Rust BE WebSocket debug processing failed'
                    )
                  );
                }
              }
  ...
  async generateVoxelCenters(
    params: VoxelDownsampleDebugBERustParams
  ): Promise<VoxelDownsampleDebugBERustResult> {
    Log.Info(
      'VoxelDownsampleDebugBERust',
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeoutId = setTimeout(() => {
        ...
        if (pending) {
          ...
          pending.reject(
            new Error(
              `Rust BE WebSocket debug timeout after ${timeoutMs / 1000}s (${pointCount.toLocaleString()} points)`
            )
          );
        }
      }, timeoutMs);
