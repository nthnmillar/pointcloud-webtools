...
export class VoxelDownsampleDebugWASMCPP extends BaseService {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      // @ts-ignore - TypeScript cannot resolve relative path module declarations for WASM files
      ...
      Log.Info(
        'VoxelDownsampleDebugWASMCPP',
        'C++ WASM module loaded via dynamic import'
      );
    } catch (error) {
      Log.Error(
        'VoxelDownsampleDebugWASMCPP',
        'Failed to load C++ WASM module',
        error
      );
      this.isInitialized = false;
      throw new Error(
        'C++ WASM module required for benchmarking - no fallback allowed'
      );
    }
  }
  ...
  async generateVoxelCenters(
    ...
    if (!this.isInitialized || !this.module) {
      try {
        ...
        Log.Error(
          'VoxelDownsampleDebugWASMCPP',
          'Initialization failed on first use',
          e
        );
        return {
          success: false,
    ...
    Log.Info(
      'VoxelDownsampleDebugWASMCPP',
      'Using C++ WASM module for voxel debug generation',
      {
    ...
    try {
      ...
      try {
        ...
        const centersArray = this.module.HEAPF32.subarray(
          outputFloatIndex,
        ...
        Log.Info(
          'VoxelDownsampleDebugWASMCPP',
          'Voxel centers generated using C++ WASM (direct)',
          {
      ...
      Log.Error(
        'VoxelDownsampleDebugWASMCPP',
        'C++ WASM voxel centers generation failed',
        error
      );
      return {
        success: false,
