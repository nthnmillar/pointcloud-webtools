...
export class VoxelDownsampleDebugTS extends BaseService {
  ...
  async generateVoxelCenters(
    ...
    try {
          ...
          pointCount > 0
            ? {
                firstPoint: {
                  ...
                  voxelX: Math.floor(
                    (params.pointCloudData[0] - minX) * invVoxelSize
                  ),
                  voxelY: Math.floor(
                    (params.pointCloudData[1] - minY) * invVoxelSize
                  ),
                  voxelZ: Math.floor(
                    (params.pointCloudData[2] - minZ) * invVoxelSize
                  ),
                },
              }
            : null,
      ...
      Log.Error(
        'VoxelDownsampleDebugTS',
        'Voxel centers generation failed',
        error
      );
      return {
        success: false,
