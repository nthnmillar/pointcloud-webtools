...
export class PointCloudSmoothingTS extends BaseService {
  ...
  async pointCloudSmoothing(params: PointCloudSmoothingParams): Promise<PointCloudSmoothingResult> {
    try {
      ...
      // Find bounding box
      let minX = smoothedPoints[0], maxX = smoothedPoints[0];
      let minY = smoothedPoints[1], maxY = smoothedPoints[1];
      let minZ = smoothedPoints[2], maxZ = smoothedPoints[2];
      ...
      for (let iter = 0; iter < params.iterations; iter++) {
        ...
        const grid: number[][] = Array(gridWidth * gridHeight * gridDepth).fill(null).map(() => []);
        
        // Populate grid with current point positions
        for (let i = 0; i < pointCount; i++) {
          const i3 = i * 3;
          const gridIndex = getGridIndex(tempPoints[i3], tempPoints[i3 + 1], tempPoints[i3 + 2]);
          ...
          for (let dx = -1; dx <= 1; dx++) {
            for (let dy = -1; dy <= 1; dy++) {
              for (let dz = -1; dz <= 1; dz++) {
                const gridIndex = getGridIndex(x + dx * cellSize, y + dy * cellSize, z + dz * cellSize);
                if (gridIndex >= 0 && gridIndex < grid.length) {
                  for (const j of grid[gridIndex]) {
                    if (i === j) continue;
