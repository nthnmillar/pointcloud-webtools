...
export class VoxelDownsamplingWASMRust extends BaseService {
  ...
  private voxelDownsampleDirectStaticFunc:
    | ((...args: number[]) => number)
    | null = null; // Cached static function
  ...
  async performVoxelDownsampling(
    ...
    try {
      ...
      if (
        ...
        this.wasmInstance.__wbindgen_export_1(
          this.previousOutputPtr,
          this.previousOutputSize,
          4
        );
        this.previousOutputPtr = null;
      ...
      const inputPtr =
        this.wasmInstance.__wbindgen_export_0(floatCount * 4, 4) >>> 0;
      const outputPtr =
        this.wasmInstance.__wbindgen_export_0(floatCount * 4, 4) >>> 0;
      ...
      try {
        ...
        if (!this.voxelDownsampleDirectStaticFunc) {
          throw new Error(
            'voxel_downsample_direct_static function not available'
          );
        }
        ...
        const downsampledPoints = this.heapF32.subarray(
          outputFloatIndex,
