...
export class VoxelDownsamplingBERust extends BaseService {
  ...
  private pendingRequests = new Map<
    string,
    {
      resolve: (value: VoxelDownsamplingBERustResult) => void;
  ...
  private connect(): void {
    try {
      Log.Info('VoxelDownsamplingBERust', 'Connecting to WebSocket', {
            ...
            if (message.type === 'voxel_downsample_rust_result') {
              if (message.success && message.dataLength) {
                ...
                if (pending) {
                  ...
                  pending.reject(
                    new Error(error || 'Rust BE WebSocket processing failed')
                  );
                }
              }
            }
  ...
  async voxelDownsample(
    params: VoxelDownsamplingBERustParams
  ): Promise<VoxelDownsamplingBERustResult> {
    Log.Info(
      'VoxelDownsamplingBERust',
