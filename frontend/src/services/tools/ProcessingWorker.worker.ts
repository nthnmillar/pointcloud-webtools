// Web Worker for point cloud processing tools (Rust WASM) - Classic Worker
...
const WorkerLog = {
  info: (message: string, data?: any) => {
    if (process.env.NODE_ENV === 'development') {
      console.log(`[ProcessingWorker] ${message}`, data || '');
    }
  },
  error: (message: string, data?: any) => {
    if (process.env.NODE_ENV === 'development') {
      console.error(`[ProcessingWorker] ${message}`, data || '');
    }
  }
};

// Initialize WASM module using fetch + eval approach (like C++ worker)
...
WorkerLog.info('ProcessingWorker Classic Worker loaded');