...
echo "Building Cython extension for voxel downsampling..."
...
ls -lh voxel_downsample_cython*.so 2>/dev/null || ls -lh voxel_downsample_cython*.pyd 2>/dev/null || echo "Extension file not found (check build output)"
