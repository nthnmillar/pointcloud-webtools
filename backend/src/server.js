...
class ProcessPool {
  ...
  async initialize() {
    ...
    console.log(
      `🔧 ProcessPool: Initialized ${this.poolSize} processes for ${this.executablePath}`
    );
  }
    ...
    try {
      ...
      try {
        ...
        if (message.type === 'test') {
          console.log(
            '🔧 Backend: Received test message from frontend:',
            message.message
          );
          ws.send(
            JSON.stringify({
          ...
          // Store header and wait for binary data
          console.log(
            '🔧 Backend: Setting pendingHeader for voxel_downsample_rust:',
            message
          );
          pendingHeader = message;
          ...
          // Store header and wait for binary data
          console.log(
            '🔧 Backend: Setting pendingHeader for point_smooth_cpp:',
            message
          );
          pendingHeader = message;
          ...
          // Store header and wait for binary data
          console.log(
            '🔧 Backend: Setting pendingHeader for point_smooth_python:',
            message
          );
          pendingHeader = message;
          ...
          // Store header and wait for binary data
          console.log(
            '🔧 Backend: Setting pendingHeader for voxel_debug_python:',
            message
          );
          pendingHeader = message;
          ...
          // Store header and wait for binary data
          console.log(
            '🔧 Backend: Setting pendingHeader for voxel_downsample_python:',
            message
          );
      ...
      if (data instanceof Buffer) {
        ...
        if (pendingHeader) {
          ...
          const pointsAlt = new Float32Array(
            data.buffer.slice(
              data.byteOffset,
          ...
          if (type === 'voxel_downsample') {
            // Extended protocol: hasColors, hasIntensity, hasClassification from header; binary = positions [+ colors][+ intensities][+ classifications]
            ...
            // Extended binary header: 36 bytes (32 + 4 for flags)
            ...
            // Binary payload: positions (and optionally colors, intensities, classifications) as sent by client
            ...
            // Convert Float32Array to Buffer for point data (binary, no JSON!)
            const pointDataBuffer = Buffer.from(
              pointsFloat32.buffer,
              pointsFloat32.byteOffset,
              pointsFloat32.byteLength
            );
            ...
            rustProcess.stderr.on('data', data => {
              ...
              if (errorText.trim()) {
                console.error(
                  '🔧 WebSocket: Rust point smooth stderr:',
                  errorText
                );
              }
            });
            ...
            rustProcess.on('error', error => {
              console.error(
                '🔧 WebSocket: Rust point smooth process error:',
                error
              );
              ws.send(
                JSON.stringify({
            ...
            rustProcess.on('close', code => {
              console.log(
                `🔧 WebSocket: Rust point smooth process closed with code ${code}, outputBuffer.length: ${outputBuffer.length}`
              );

              if (code !== 0) {
                console.error(
                  `🔧 WebSocket: Rust point smooth process exited with code ${code}`
                );
                ws.send(
                  JSON.stringify({
                    type: 'point_smooth_rust_result',
              ...
              try {
                ...
                const smoothedPointsBuffer = outputBuffer.slice(
                  4,
                  expectedSize
                );
                const _smoothedPoints = new Float32Array(
                  smoothedPointsBuffer.buffer,
                  smoothedPointsBuffer.byteOffset,
                  outputCount * 3
                );
                ...
                console.error(
                  '🔧 WebSocket: Rust point smooth binary protocol error:',
                  parseError
                );
                console.error(
                  '🔧 WebSocket: Rust point smooth stderr:',
                ...
                console.error(
                  '🔧 WebSocket: Rust point smooth stdout length:',
                  outputBuffer.length
                );
                ws.send(
                  JSON.stringify({
            ...
            console.log(
              '🔧 WebSocket: Writing',
              inputBuffer.length,
              'bytes to Rust point smooth stdin'
            );
            rustProcess.stdin.write(inputBuffer);
            rustProcess.stdin.end();
            ...
            cppProcess.stderr.on('data', data => {
              ...
              if (errorText.trim()) {
                console.error(
                  '🔧 WebSocket: C++ point smooth stderr:',
                  errorText
                );
              }
            });
            ...
            cppProcess.on('error', error => {
              console.error(
                '🔧 WebSocket: C++ point smooth process error:',
                error
              );
              ws.send(
                JSON.stringify({
            ...
            cppProcess.on('close', code => {
              console.log(
                `🔧 WebSocket: C++ point smooth process closed with code ${code}, outputBuffer.length: ${outputBuffer.length}`
              );

              if (code !== 0) {
                console.error(
                  `🔧 WebSocket: C++ point smooth process exited with code ${code}`
                );
                ws.send(
                  JSON.stringify({
                    type: 'point_smooth_cpp_result',
              ...
              try {
                ...
                console.error(
                  '🔧 WebSocket: C++ point smooth binary protocol error:',
                  parseError
                );
                console.error(
                  '🔧 WebSocket: C++ point smooth stderr:',
                ...
                console.error(
                  '🔧 WebSocket: C++ point smooth stdout length:',
                  outputBuffer.length
                );
                ws.send(
                  JSON.stringify({
            ...
            const pythonExecutable = path.join(
              ...
              'python',
              'point_smooth',
              'point_smooth_cython_wrapper.py'
            );
            ...
            try {
              ...
              console.error(
                '🔧 WebSocket: Failed to spawn Python process:',
                spawnError
              );
              ws.send(
                JSON.stringify({
                  ...
                  success: false,
                  error:
                    'Failed to spawn Python process: ' + spawnError.message,
                })
              );
              return;
            }
            ...
            pythonProcess.stderr.on('data', data => {
              ...
              if (errorText.trim()) {
                console.error(
                  '🔧 WebSocket: Python point smooth stderr:',
                  errorText
                );
              }
            });
            ...
            pythonProcess.on('error', error => {
              console.error(
                '🔧 WebSocket: Python point smooth process error:',
                error
              );
              ws.send(
                JSON.stringify({
            ...
            pythonProcess.on('close', code => {
              console.log(
                `🔧 WebSocket: Python point smooth process closed with code ${code}, outputBuffer.length: ${outputBuffer.length}`
              );

              if (code !== 0) {
                console.error(
                  `🔧 WebSocket: Python point smooth process exited with code ${code}`
                );
                ws.send(
                  JSON.stringify({
                    type: 'point_smooth_python_result',
              ...
              try {
                ...
                console.error(
                  '🔧 WebSocket: Python point smooth binary protocol error:',
                  parseError
                );
                console.error(
                  '🔧 WebSocket: Python point smooth stderr:',
                ...
                console.error(
                  '🔧 WebSocket: Python point smooth stdout length:',
                  outputBuffer.length
                );
                ws.send(
                  JSON.stringify({
            ...
            console.log(
              '🔧 WebSocket: Writing',
              inputBuffer.length,
              'bytes to Python point smooth stdin'
            );
            ...
            try {
              ...
              console.error(
                '🔧 WebSocket: Failed to spawn C++ process:',
                spawnError
              );
              ws.send(
                JSON.stringify({
            ...
            cppProcess.stderr.on('data', data => {
              ...
              if (errorText.trim()) {
                console.error(
                  '🔧 WebSocket: C++ voxel debug stderr:',
                  errorText
                );
              }
            });
            ...
            cppProcess.on('error', error => {
              console.error(
                '🔧 WebSocket: C++ voxel debug process error:',
                error
              );
              ws.send(
                JSON.stringify({
            ...
            cppProcess.on('close', code => {
              console.log(
                `🔧 WebSocket: C++ voxel debug process closed with code ${code}, outputBuffer.length: ${outputBuffer.length}`
              );

              if (code !== 0) {
                console.error(
                  `🔧 WebSocket: C++ voxel debug process exited with code ${code}`
                );
                ws.send(
                  JSON.stringify({
                    type: 'voxel_debug_cpp_result',
              ...
              try {
                ...
                const voxelGridPositionsBuffer = outputBuffer.slice(
                  4,
                  expectedSize
                );
                const _voxelGridPositions = new Float32Array(
                  voxelGridPositionsBuffer.buffer,
                  voxelGridPositionsBuffer.byteOffset,
                  voxelCount * 3
                );
                ...
                console.error(
                  '🔧 WebSocket: C++ voxel debug binary protocol error:',
                  parseError
                );
                console.error(
                  '🔧 WebSocket: C++ voxel debug stderr:',
                ...
                console.error(
                  '🔧 WebSocket: C++ voxel debug stdout length:',
                  outputBuffer.length
                );
                ws.send(
                  JSON.stringify({
            ...
            try {
              ...
              console.error(
                '🔧 WebSocket: Failed to spawn Rust process:',
                spawnError
              );
              ws.send(
                JSON.stringify({
            ...
            rustProcess.stderr.on('data', data => {
              ...
              if (errorText.trim()) {
                console.error(
                  '🔧 WebSocket: Rust voxel debug stderr:',
                  errorText
                );
              }
            });
            ...
            rustProcess.on('error', error => {
              console.error(
                '🔧 WebSocket: Rust voxel debug process error:',
                error
              );
              ws.send(
                JSON.stringify({
            ...
            rustProcess.on('close', code => {
              console.log(
                `🔧 WebSocket: Rust voxel debug process closed with code ${code}, outputBuffer.length: ${outputBuffer.length}`
              );

              if (code !== 0) {
                console.error(
                  `🔧 WebSocket: Rust voxel debug process exited with code ${code}`
                );
                ws.send(
                  JSON.stringify({
                    type: 'voxel_debug_rust_result',
              ...
              try {
                ...
                console.error(
                  '🔧 WebSocket: Rust voxel debug binary protocol error:',
                  parseError
                );
                console.error(
                  '🔧 WebSocket: Rust voxel debug stderr:',
                ...
                console.error(
                  '🔧 WebSocket: Rust voxel debug stdout length:',
                  outputBuffer.length
                );
                ws.send(
                  JSON.stringify({
            ...
            console.log(
              '🔧 WebSocket: Writing',
              inputBuffer.length,
              'bytes to Rust voxel debug stdin'
            );
            rustProcess.stdin.write(inputBuffer);
            rustProcess.stdin.end();
            ...
            console.log(
              '🔧 WebSocket: Processing Python BE request with type:',
              type
            );
            ...
            const pythonExecutable = path.join(
              ...
              'python',
              'voxel_debug',
              'voxel_debug_cython_wrapper.py'
            );
            console.log(
              '🔧 WebSocket: Python executable path:',
              pythonExecutable
            );
            ...
            pythonProcess.stderr.on('data', data => {
              ...
              if (errorText.trim()) {
                console.error(
                  '🔧 WebSocket: Python voxel debug stderr:',
                  errorText
                );
              }
            });
            ...
            pythonProcess.on('close', code => {
              console.log(
                `🔧 WebSocket: Python voxel debug process closed with code ${code}, outputBuffer.length: ${outputBuffer.length}`
              );

              if (code !== 0) {
                console.error(
                  `🔧 WebSocket: Python process exited with code ${code}`
                );
                ws.send(
                  JSON.stringify({
                    type: 'voxel_debug_python_result',
              ...
              try {
                ...
                console.error(
                  '🔧 WebSocket: Python voxel debug binary protocol error:',
                  parseError
                );
                console.error(
                  '🔧 WebSocket: Python voxel debug stderr:',
                ...
                console.error(
                  '🔧 WebSocket: Python voxel debug stdout length:',
                  outputBuffer.length
                );
                ws.send(
                  JSON.stringify({
            ...
            console.log(
              '🔧 WebSocket: Writing',
              inputBuffer.length,
              'bytes to Python voxel debug stdin'
            );
            pythonProcess.stdin.write(inputBuffer);
            pythonProcess.stdin.end();
            ...
            console.log(
              '🔧 WebSocket: Starting Python voxel downsampling process'
            );
            console.log(
              '🔧 WebSocket: Processing Python BE request with type:',
              type
            ...
            const pythonExecutable = path.join(
              ...
              'python',
              'voxel_downsample',
              'voxel_downsample_cython_wrapper.py'
            );
            console.log(
              '🔧 WebSocket: Python executable path:',
              pythonExecutable
            ...
            pythonProcess.stderr.on('data', data => {
              ...
              if (errorText.trim()) {
                console.error(
                  '🔧 WebSocket: Python voxel downsampling stderr:',
                  errorText
                );
              }
            });
            ...
            pythonProcess.on('close', code => {
              console.log(
                `🔧 WebSocket: Python voxel downsampling process closed with code ${code}, outputBuffer.length: ${outputBuffer.length}`
              );
              ...
              try {
                ...
                console.error(
                  '🔧 WebSocket: Python voxel downsampling binary protocol error:',
                  parseError
                );
                console.error(
                  '🔧 WebSocket: Python voxel downsampling stderr:',
                ...
                console.error(
                  '🔧 WebSocket: Python voxel downsampling stdout length:',
                  outputBuffer.length
                );
                ws.send(
                  JSON.stringify({
            ...
            console.log(
              '🔧 WebSocket: Writing',
              inputBuffer.length,
              'bytes to Python voxel downsampling stdin'
            );
            pythonProcess.stdin.write(inputBuffer);
            pythonProcess.stdin.end();
          }
...
app.use(
  cors({
    origin: [
      ...
      'http://localhost:3000',
      'http://localhost:8080',
      'http://44.218.34.156:3000',
      'http://44.218.34.156',
      'http://pointcloud-webtools.nathanmillar.net',
      'https://pointcloud-webtools.nathanmillar.net',
      /^http:\/\/.*\.amazonaws\.com/,
      /^http:\/\/.*\.lightsail\.aws/,
      /^https?:\/\/.*\.nathanmillar\.net/,
    ],
    credentials: true,
  })
);
...
app.post('/api/voxel-downsample', async (req, res) => {
  try {
    ...
    cppProcess.on('close', (code, signal) => {
      console.log(
        `🔧 C++ process closed with code ${code}, signal ${signal}, outputBuffer.length: ${outputBuffer.length}, errorBuffer length: ${errorBuffer.length}`
      );
      ...
      if (outputBuffer.length === 0) {
        ...
        if (!res.headersSent) {
          res.status(500).json({
            success: false,
            error:
              'C++ process produced no output. Stderr: ' +
              (errorBuffer || 'none'),
          });
        }
        return;
      ...
      try {
        ...
        console.log(
          `🔧 Sending success response: ${outputCount} points, ${processingTime}ms`
        );
        if (!res.headersSent) {
          res.json({
            success: true,
...
app.post('/api/point-smooth', async (req, res) => {
  try {
    ...
    console.log(
      '🔧 Backend: Using real C++ backend processing for point cloud smoothing'
    );
    ...
    cppProcess.stdout.on('end', () => {
      ...
      if (lines.length >= 2) {
        ...
        console.log(
          '🔧 Backend: Parsed smoothedPoints:',
          smoothedPoints.length / 3,
          'points'
        );
      }
      ...
      const processingTime = Date.now() - startTime;
      console.log(
        '🔧 Backend: C++ point cloud smoothing processing completed',
        {
...
app.post('/api/voxel-debug', async (req, res) => {
  try {
    ...
    cppProcess.on('close', code => {
      console.log(
        `🔧 C++ voxel debug process closed with code ${code}, outputBuffer.length: ${outputBuffer.length}`
      );
      ...
      try {
        ...
        console.log(
          `🔧 C++ voxel debug success: ${voxelCount} voxels, ${processingTime}ms`
        );
        if (!res.headersSent) {
          res.json({
            success: true,
...
app.post('/api/voxel-downsample-rust', async (req, res) => {
  try {
    ...
    console.log(
      '🔧 Backend: Using Rust backend processing for voxel downsampling'
    );
...
app.post('/api/point-smooth-rust', async (req, res) => {
  try {
    ...
    // Use Rust backend processing
    console.log(
      '🔧 Backend: Using Rust backend processing for point cloud smoothing'
    );
...
app.post('/api/voxel-debug-rust', async (req, res) => {
  try {
    ...
    rustProcess.on('close', code => {
      console.log(
        `🔧 Rust voxel debug process closed with code ${code}, outputBuffer.length: ${outputBuffer.length}`
      );
      ...
      try {
        ...
        console.log(
          `🔧 Rust voxel debug success: ${voxelCount} voxels, ${processingTime}ms`
        );
        if (!res.headersSent) {
          res.json({
            success: true,
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(
    `Voxel downsampling: http://localhost:${PORT}/api/voxel-downsample`
  );
...
server.on('error', err => {
  if (err.code === 'EADDRINUSE') {
    console.log(
      `Port ${PORT} is already in use. Backend server may already be running.`
    );
